...
var_ids : ID {
              map<string, id_info>::iterator it = symbol_table.find($1);
              if(it == symbol_table.end())
                 symbol_table[$1] = var_info(++local_vars[level], level);
              else
                 cerr << "ERROR: (line " << yylineno << "): " << $1 << " is already declared";
              $$ = "";
           }
...
proc_decls : { $$=""; }
           | proc proc_decls {$$ = $1+$2;}
           ;
proc : PROC ID SEMICOLON {local_vars[++level]=0;} const_decls var_decls  {
           map<string, id_info>::iterator it = symbol_table.find($2);
           if(it == symbol_table.end())
              symbol_table[$2] = proc_info(local_vars[level], level-1);
           else
